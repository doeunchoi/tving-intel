...
| 증상 | 원인 · 조치 |
|---|---|
| Actions 로그에 `FAIL: API key does not have access to this project` | API 키가 무효화됨(키 폐기, 발급자 이탈, 프로젝트 Transfer 등). 위 인수인계 절차대로 키 재발급 + Secrets 갱신 후 **Actions → framer-sync → Run workflow** 로 수동 실행하면 밀린 변경분이 한 번에 따라잡힘(멱등) |
| 실패가 계속되는데 아무도 모름 | 워크플로가 실패 시 자동으로 GitHub 이슈를 생성/댓글 누적함 — 레포 이슈 알림을 켜 두세요. (실제로 2026-06-25 키 무효화 후 일주일간 미동기화된 사고가 있었음) |
